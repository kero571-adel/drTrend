...
export default function Checkout() {
  ...
  const { user, loading: authLoading } = useAuth(); // ← أضفنا authLoading
  ...
  // Auth + cart gate — بنستنى authLoading الأول عشان Firebase يأكد الـ session
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // حفظ العنوان في localStorage لو اختار
      if (saveAddress) {
      ...
      const order: Order = {
        ...
        shipping: shipping || 0,
        total,
        grandTotal: subtotal + (shipping || 0),   // ← أضف
        placedAt: new Date().toISOString(),        // ← أضف
      ...
      // ← رفع الأوردر على Firestore (بدل localStorage)
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[60%_40%] gap-10">
        <form
          ...
          className="bg-white rounded-xl border border-gray-200 p-6 md:p-8 space-y-5"
        >
          ...
          {/* ← شيلنا onClick من هنا — handleSubmit بتعمل كل حاجة */}
          <button
            type="submit"
            disabled={submitting}
